...
const LimitedProduct = ({
  ifExists,
  endpoint: { heading, title, text, img, btn, rating, price },
}) => {
  return (
    <>
      <div
      ...
      >
        <div className='max-w-lg lg:max-w-none w-full md:text-center grid items-center lg:justify-items-center'>
          <h1 className='text-5xl sm:text-3xl font-bold text-gradient'>
            {heading}
          </h1>
          <h1 className='text-4xl lg:text-4xl md:text-3xl sm:text-2xl font-bold text-slate-900 filter drop-shadow-lg'>
            {title}
          </h1>
        ...
        <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
          <img
            src={img}
            alt={`img/${heading}`}
            className={`w-auto object-fill md:mt-4 sm:mt-6 transitions-theme ${
              ifExists
